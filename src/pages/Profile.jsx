...
const recentActivity = [
  { emoji: "📓", item: "Spiral Notebook", date: "Mar 12, 2026", price: 120 },
  { emoji: "🖊️", item: "Ballpen Set",     date: "Mar 8, 2026",  price: 85  },
  { emoji: "📐", item: "Ruler & Compass", date: "Feb 28, 2026", price: 210 },
];
...
function Profile({ onProfileUpdate }) {
    ...
    try {
      ...
      if (data) {
        ...
        const defaultProfile = {
          user_id: user.id,
          first_name: user.user_metadata?.first_name || "Campus",
        ...
        if (insertError) console.error("Profile insert:", insertError);
  ...
  const handleAvatarChange = async (e) => {
    ...
    try {
      ...
      // Fallback: show locally even if storage fails
      const reader = new FileReader();
      reader.onload = (ev) => setAvatarSrc(ev.target.result);
      reader.readAsDataURL(file);
      showToast("Photo updated locally (storage not configured).", "info");
    } finally {
      ...
      <div className="profile-inner">
        ...
        <div className="profile-left">
          <div className="avatar-card">
            <div className="avatar-ring" onClick={() => avatarInputRef.current?.click()} title="Click to change photo">
            ...
            <button className="edit-btn" onClick={() => { setForm({ ...profile }); setModalOpen(true); }}>
              Edit Profile
            </button>
          </div>
        </div>
        ...
        <div className="profile-right">
          <div className="stats-row">
            ...
            <div className="stat-card accent">
              <span className="stat-value">₱{(profile.spent || 0).toLocaleString()}</span>
            ...
            <div className="modal-form">
              <div className="form-row-grid">
                <div className="form-field">
                  ...
                  <input name="first_name" value={form.first_name || ""} onChange={(e) => setForm(p => ({ ...p, first_name: e.target.value }))} placeholder="First name" />
                </div>
                <div className="form-field">
                  <label>Last Name</label>
                  <input name="last_name" value={form.last_name || ""} onChange={(e) => setForm(p => ({ ...p, last_name: e.target.value }))} placeholder="Last name" />
                </div>
              </div>

              <div className="form-field">
                <label>Email</label>
                <input name="email" type="email" value={form.email || ""} onChange={(e) => setForm(p => ({ ...p, email: e.target.value }))} placeholder="Email address" />
              </div>

              <div className="form-row-grid">
                <div className="form-field">
                  <label>Role</label>
                  <select name="role" value={form.role || "Student"} onChange={(e) => setForm(p => ({ ...p, role: e.target.value }))}>
                ...
                <div className="form-field">
                  ...
                  <select name="year" value={form.year || ""} onChange={(e) => setForm(p => ({ ...p, year: e.target.value }))}>
              ...
              <div className="form-field">
                <label>Course</label>
                <input name="course" value={form.course || ""} onChange={(e) => setForm(p => ({ ...p, course: e.target.value }))} placeholder="Course / Program" />
              </div>

              <div className="modal-actions">
                <button className="btn-cancel" onClick={() => setModalOpen(false)} disabled={saving}>Cancel</button>
